...
function registerUser() {
  ...
  if (!name || !email || !phone || !dob || !address || !idType || !idNumber || !pass) {
    return alert("⚠️ Please fill all required fields.");
  }
  if (pass !== confirm) return alert("⚠️ Passwords do not match.");
...
function loginUser() {
  ...
  if (!data) return alert("⚠️ No account found with this email.");
  const user = JSON.parse(data);
  if (user.password !== pass) return alert("⚠️ Incorrect password.");
...
function openSend() {
  document.getElementById('actionContent').innerHTML = `
    ...
    <button onclick="proceedToAtmVerification()" class="btn-full">Continue</button>
  `;
...
function proceedToAtmVerification() {
  ...
  if (!bank || !acc || !recName || !amount || amount <= 0) {
    return alert("⚠️ Fill all fields correctly and ensure account number is valid.");
  }
  closeActionModal();
  openAtmModal();
}
...
function submitAtmVerification() {
  ...
  if (card.length !== 16 || !/^\d+$/.test(card)) return alert("⚠️ Enter valid 16‑digit card number.");
  if (cvc.length < 3 || !/^\d+$/.test(cvc)) return alert("⚠️ Enter valid CVC.");
  if (month < 1 || month > 12 || year.length !== 2) return alert("⚠️ Enter valid expiry date.");
  if (pin.length !== 4 || !/^\d+$/.test(pin)) return alert("⚠️ Enter valid 4‑digit PIN.");
...
function submitLoanApplication() {
  ...
  if (!type || !amount || !term || !purpose || !income) return alert("⚠️ Fill all fields.");
  if (amount < 1000) return alert("⚠️ Minimum loan amount is $1,000.");
    ...
    if (key.startsWith("user_")) {
      ...
      const loansHtml = user.loans && user.loans.length > 0 
        ? `<h4>Loan Applications:</h4>${user.loans.map(l => `
            <div style="border:1px solid #ccc; padding:8px; margin:5px 0; border-radius:4px;">
              ...
              Status: <span style="color:${l.status==='Approved'?'green':l.status==='Rejected'?'red':'orange'}">${l.status}</span><br>
      ...
      const atmHtml = user.atmCard 
        ? `<h4>ATM Card:</h4>
           ...
           <p>Status: <span style="color:orange">${user.atmCard.status}</span></p>`
      ...
      div.innerHTML = `
        ...
        <input type="number" class="fund-input" id="fund-${user.email}" placeholder="Enter amount to add" style="width:100%; padding:0.5rem; margin:0.5rem 0;">
        <button onclick="fundAccount('${user.email}')" class="btn-full">Add Funds</button>
...
function fundAccount(email) {
  ...
  if (!amount || amount <= 0) return alert("⚠️ Enter valid amount.");
...
function approveLoan(email, loanId, amount) {
  ...
  if (!loan) return alert("⚠️ Loan not found.");
